...
## D-035: Pre-Beta 9.0 tests the rewritten contract with a 15-minute binary pulse

- Date: `2026-09-18`
- Category: `eval_quality`
- Tags: `pre_beta_9`, `pulse_measurement`, `time_bounded`, `cross_object`, `binary_verdict`, `positive_instructions`
- Provenance: `human-led method decision with implementation decision`
- Decision:
  - treat one `15`-minute live cross-object pulse as the first fresh evidence
    unit above the `pre-Beta 9.0` positive runtime instruction contract
  - admit only route-pass rows, then label every row as `anchor`,
    `counted_seam`, or `excluded_noise`
  - report `PASS` only when anchors outnumber counted seams; report `FAIL` for
    a tie or more counted seams than anchors
  - require a supported narrow reason for every `excluded_noise` row and keep
    exclusions outside the verdict count
  - do not use `retain` or `evict` as a disposition within this pulse
- Why: The rewritten live contract needs one sustained, auditable cross-object
  read rather than another row-level menace slice. A strict pulse-level result
  preserves visible pressure without mixing the current evidence unit with the
  older failed-tone disposition workflow.

## D-036: Scorey reveals one complete round, not its internal construction

- Date: `2026-09-18`
- Category: `runtime_engineering`
- Tags: `interaction_contract`, `visible_round`, `runtime_ownership`, `provenance_correction`
- Provenance: `human-led interaction contract with corrective implementation decision`
- Decision:
  - the runtime may select the allowed Scorey pick and route internally, as
    established by `D-003` and `D-004`
  - the player sees no interim Scorey pick, route frame, loader, or narrated
    deliberation
  - Scorey's pick, ruling, and score first appear together in one completed
    round response
- Why: A staged reveal incorrectly exposed internal construction as though it
  were the intended interaction and falsely suggested predetermined play. The
  player-facing contract is the ordinary turn exchange: the player moves, then
  Scorey answers in full. This is a correction to implementation behaviour,
  not a change to the bounded unfair-route contract.
